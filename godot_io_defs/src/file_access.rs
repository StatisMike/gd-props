use std::{io::{Read, Write, Seek, BufRead, Error as IoError, ErrorKind}, cmp};

use godot::{prelude::Gd, engine::{FileAccess, global::Error}};
...
impl FaWrapper {
  ...
  fn check_error(&self) -> Result<(), IoError>{
    ...
    Err(IoError::new(ErrorKind::Other, format!("GodotError: {:?}", self.fa.get_error())))
  }
}
...
impl Read for FaWrapper {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
      ...
      if self.pos >= length {
        // This is for methods that consume the `R` - to make sure 
        // that the file is closed after reading is done. Don't really
        // like it, but don't know if there is another way
        self.fa.close();
        return Ok(0);
      }
        ...
        match pos {
            ...
            std::io::SeekFrom::End(position) => {
              if (self.fa.get_length() as i64) < position {
                return Err(IoError::new(ErrorKind::InvalidInput, "Position set would be negative"));
              }
