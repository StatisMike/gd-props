use godot::{
  engine::{global::Error, Engine, ResourceFormatSaver, ResourceUid},
  obj::{cap::GodotInit, dom::UserDomain, Gd},
  prelude::{godot_error, GodotClass, GodotString, Inherits, Object},
};

use crate::gd_meta::GdMetaHeader;

pub trait GdBinSaver
where
      ...
      + Inherits<Object>,
{
  ...
  ///             MyBinSaverStruct::register_saver();
  ///         }   
  ///     }
  /// }
  /// ```
  ...
  fn _int_set_uid(&mut self, path: GodotString, uid: i64) -> Error {
      let meta_res = GdMetaHeader::read_from_gdbin_header(path.clone());
      ...
      match meta_res {
          Ok(mut meta) => {
              ...
              let write_res = meta.write_to_gdbin_header(path.clone());
